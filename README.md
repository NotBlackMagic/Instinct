...
- **`Instinct/`:** Core firmware applications, RTOS threads, vision pipelines, and control loops.
	- **`Config/`:** Configuration files for middlewares (e.g. fx_user.h, trx_user.h, etc...)
	- **`Modules/`:** Folder containing functional parts of the vehicle controller firmware Instinct.
		- **`Acquisition/`:** Sensor acquisition threads.
		- **`Actuation/`:** Mapping output commands to peripherals (e.g. PWM channels).
		- **`Commander/`:** System and remote control commands to kinematic commands.
		- **`Control/`:** Control loops, PID system to execute the kinematic commands.
		- **`Estimator/`:** Invariant EKF based state estimator.
		- **`Math/`:** Common math library (matrix, rotation, pid, etc...).
