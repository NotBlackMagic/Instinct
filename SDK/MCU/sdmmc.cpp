...
/*
bool SDMMC::PrepareData(uint8_t *buf, uint32_t len, uint32_t blkSize, bool isRead) {
	// Calculate block size exponent (for SDMMC_DCTRL_DBLOCKSIZE)
	uint32_t blkSizeLog2 = 0;
	if(blkSize > 0) {
		blkSizeLog2 = 31UL - __CLZ(blkSize);
	...
	uint32_t transferDir = SDMMC_TRANSFER_DIR_TO_CARD;		//SDMMC_TRANSFER_DIR_TO_CARD or SDMMC_TRANSFER_DIR_TO_SDMMC
	uint32_t transferMode = SDMMC_TRANSFER_MODE_BLOCK;		//SDMMC_TRANSFER_MODE_BLOCK, SDMMC_TRANSFER_MODE_SDIO or SDMMC_TRANSFER_MODE_STREAM
	uint32_t dpsm = SDMMC_DPSM_DISABLE;						//SDMMC_DPSM_DISABLE or SDMMC_DPSM_ENABLE
...
bool SDMMC::WaitForTransfer(uint32_t timeoutMs) {
	uint32_t remaining = this->length;
	uint32_t timestamp = Time::GetMs();
	...
	while(remaining > 0) {
		...
		// Check Errors
		if((status & (SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_TXUNDERR | SDMMC_FLAG_RXOVERR | SDMMC_FLAG_DTIMEOUT)) != 0) {
			// Clear all the static flags
			MODIFY_REG(this->instance->ICR, SDMMC_STATIC_DATA_FLAGS, SDMMC_STATIC_DATA_FLAGS);
			return false;
		}
		...
		if(this->dirRead == false) {
			// Handle write
			if(((this->instance->STA & (SDMMC_FLAG_TXFIFOHE)) == SDMMC_FLAG_TXFIFOHE)) {
				uint32_t burstCnt = (remaining < SDMMC_FIFO_SIZE) ? remaining : SDMMC_FIFO_SIZE;
				uint32_t i;
				for(i = 0; i < burstCnt; i += 4) {
					// Write to FIFO, packed 32-bit words
					uint32_t data = (uint32_t)(*this->buffer);
					this->buffer++;
					data |= ((uint32_t)(*this->buffer) << 8);
					this->buffer++;
					data |= ((uint32_t)(*this->buffer) << 16);
					this->buffer++;
					data |= ((uint32_t)(*this->buffer) << 24);
					this->buffer++;
					this->instance->FIFO = data;
				}
				remaining -= burstCnt;
			}
		}
			...
			// Handle read
			if(((this->instance->STA & (SDMMC_FLAG_RXFIFOHF)) == SDMMC_FLAG_RXFIFOHF)) {
				uint32_t burstCnt = (remaining < SDMMC_FIFO_SIZE) ? remaining : SDMMC_FIFO_SIZE;
				uint32_t i;
				for(i = 0; i < burstCnt; i += 4) {
					uint32_t data = this->instance->FIFO;
					*this->buffer = (uint8_t)(data & 0xFF);
					this->buffer++;
					*this->buffer = (uint8_t)((data >> 8) & 0xFF);
					this->buffer++;
					*this->buffer = (uint8_t)((data >> 16) & 0xFF);
					this->buffer++;
					*this->buffer = (uint8_t)((data >> 24) & 0xFF);
					this->buffer++;
				}
				remaining -= burstCnt;
			}
			else if(((this->instance->STA & SDMMC_FLAG_RXFIFOE) == 0x00) && (remaining > 0)) {
				while(remaining >= 4 && ((this->instance->STA & SDMMC_FLAG_RXFIFOE) == 0x00)) {
					uint32_t data = this->instance->FIFO;
					*this->buffer = (uint8_t)(data & 0xFF);
					this->buffer++;
					*this->buffer = (uint8_t)((data >> 8) & 0xFF);
	...
	// Wait for end signal
	while(((this->instance->STA & (SDMMC_FLAG_DATAEND)) == 0x00)) {
		status = this->instance->STA;
		if((status & (SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT)) != 0x00) {
			// Clear all the static flags
			MODIFY_REG(this->instance->ICR, SDMMC_STATIC_DATA_FLAGS, SDMMC_STATIC_DATA_FLAGS);
			return false;
		}
